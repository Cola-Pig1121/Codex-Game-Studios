...
After decomposing all stories (Step 4 complete) but before presenting them for write Verification, spawn `qa-lead` via Task using gate **QL-STORY-READY** (`.codex/docs/director-gates.md`).
...
Use `AskUserQuestion`:
- Prompt: "I will write these [N] stories to `production/epics/[epic-slug]/`?"
...
4. **Ask before writing** — get Verification for the full story set before writing files

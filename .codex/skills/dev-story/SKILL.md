...
3. If any dependency has Status other than `Complete` or `Done`:
   ...
   - If [C]: ask "I will update [dependency path] Status to Complete?" before continuing.
...
- **File writes are delegated** — all source code, test files, and evidence docs are written by sub-agents spawned via Task. Each sub-agent enforces the "I will write to [path]?" protocol individually. This orchestrator does not write files directly.

...
```
Context  ->  Questions  ->  Options  ->  Decision  ->  Draft  ->  Verification  ->  Write
```
...
6. **Verification**: Immediately after the draft — in the SAME response — use
   ...
   **The draft and the Verification widget MUST appear together in one response.
...
Ask: "I will update `design/registry/entities.yaml` with these [N] new entries
...
Ask: "I will update the systems index at `design/gdd/systems-index.md`?"
...
1. **Inspect -> Options when needed -> Decide -> Draft -> Verify** for every section
2. **AskUserQuestion** at every decision point (Explain -> Capture pattern):
   - Phase 2: "Ready to start, or need more context?"
   - Phase 3: "May I create the skeleton?"
   - Phase 4 (each section): Design questions, approach options, draft Verification
   - Phase 5: "Run design review? Update systems index? What's next?"
3. **"I will write to [filepath]?"** before the skeleton and before each section write
4. **Incremental writing**: Each section is written to file immediately after verification
...
**Never** write a section without user Verification.

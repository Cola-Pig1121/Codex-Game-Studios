...
4.7. **GDD Sync Check** — Before presenting the write Verification, scan all GDDs
...
immediately before the write Verification — not as a footnote:
...
5. **Write Verification** — Use `AskUserQuestion`:
...
- "ADR draft is complete. I will write it?"
...
**BLOCKING — do not write to `docs/registry/architecture.yaml` without verification.**

Ask using `AskUserQuestion`:
- "I will update `docs/registry/architecture.yaml` with these [N] new stances?"

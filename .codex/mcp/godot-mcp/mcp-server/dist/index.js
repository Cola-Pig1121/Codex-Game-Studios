...
// ---------------------------------------------------------------------------
let godotBridge = null;
async function executeToolCall(name, toolArgs) {
    if (name === 'get_guide') {
    ...
    if (!toolExists(name)) {
        throw new McpError(ErrorCode.MethodNotFound, `Unknown tool: ${name}. Available tools: ${allTools.map(t => t.name).join(', ')}`);
    }
    ...
    try {
        ...
        if (name === 'map_project' && result && typeof result === 'object' && 'project_map' in result) {
            try {
                ...
                return {
                    content: [{
                            ...
                            text: JSON.stringify({
                                ...
                                message: `Project mapped: ${projectMap.total_scripts} scripts, ${projectMap.total_connections} connections. Interactive visualization opened in browser at ${url}`
                            }, null, 2)
                        }]
                };
            }
        ...
        if (details && typeof details === 'object') {
            ...
            const { ok: _ok, error: _err, ...rest } = details;
...
function createMcpServer(handleTool) {
...
async function killProcessOnPort(port) {
    try {
        ...
        if (platform === 'win32') {
            ...
            const output = execSync(`lsof -ti :${port} -sTCP:LISTEN`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
...
async function startPrimary() {
    ...
    let idleTimer = null;
    function maybeStartIdleShutdown() {
        if (idleTimer)

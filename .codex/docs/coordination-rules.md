...
|------|-------|-------------|
| **gpt-5.5 low** | `gpt-5.5 (reasoning_effort: low)` | Read-only status checks, formatting, simple lookups — no creative judgment needed |
| **gpt-5.5 medium** | `gpt-5.5 (reasoning_effort: medium)` | Implementation, design authoring, analysis of individual systems — default for most work |
| **gpt-5.5 xhigh** | `gpt-5.5 (reasoning_effort: xhigh)` | Multi-document synthesis, high-stakes phase gate verdicts, cross-system holistic review |

Skills with `model: gpt-5.5` and `reasoning_effort: low`: `/help`, `/sprint-status`, `/story-readiness`, `/scope-check`,
`/project-stage-detect`, `/changelog`, `/patch-notes`, `/onboard`

Skills with `model: gpt-5.5` and `reasoning_effort: xhigh`: `/review-all-gdds`, `/architecture-review`, `/gate-check`

All other skills default to `model: gpt-5.5` with `reasoning_effort: medium`. When creating new skills, assign `reasoning_effort: low` if the
skill only reads and formats; assign `reasoning_effort: xhigh` if it must synthesize 5+ documents with
high-stakes output; otherwise use `reasoning_effort: medium`.
